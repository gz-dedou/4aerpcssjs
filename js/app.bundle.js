		...
		try {
			...
			if (documentUrl) {
				if (documentUrl.indexOf('#', 0) > 0 && documentUrl.indexOf('#', 0) < (documentUrl.length + 1)) {
					...
					url = documentUrl + "#dashboard";
					//alert(url);
					window.location.href = url;
					return;
					//window.location.hash = '/dadmin/reviews';
				}
...
function loadURL(url, container) {
	...
	$.ajax({
		...
		beforeSend: function () {
			...
			// place cog
			container.html('<h1 class="ajax-loading-animation"><i class="fal fa-cog fa-spin"></i> 加载中...</h1>');
		...
		error: function (xhr, status, thrownError, error) {
			container.html('<h4 class="text-danger"><i class="fal fa-warning text-warning"></i> Error requesting <span class="text-danger">' + url + '</span>: ' + xhr.status + ' <span style="text-transform: capitalize;">' + thrownError + '</span></h4>');
		},
		async: true
	});
